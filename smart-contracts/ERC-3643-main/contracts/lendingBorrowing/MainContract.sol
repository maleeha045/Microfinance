...
contract MainContract is Ownable, ERC721URIStorage {
    IToken public erc3643Token;
    IERC20 public USDT;

    constructor(
        address _erc3643Token,
        address _USDT
    ) ERC721("InvoiceNFT", "INF") {
        erc3643Token = IToken(_erc3643Token);
        USDT = IERC20(_USDT);
    }
    ...
    function transferUSDT(address _to, uint _amount) public {
        bool success = USDT.transfer(_to, _amount);
        require(success, "Error transferring USDT amount to user");
    }
    ...
    function depositUSDT(
        ...
        bool success = USDT.transferFrom(_from, address(this), _usdtAmount);
        require(success, "Error transferring USDT to contract");
        USDT.transfer(_to,_principal);
    }
    function depositUSDTToBorrower(
        address _to,
        uint256 _usdtAmount) external{
        // USDT.approve(address(this), _usdtAmount);
        // USDT.approve(_to)
        bool success = USDT.transfer(_to, _usdtAmount);
        require(success, "Error transferring USDT to borrower");
    
    }
 


    function claimBackUSDT(
        address _lender,
        uint256 _totalClaimAmount
    ) external onlyOwner {
        transferUSDT(_lender, _totalClaimAmount);
    }
    // contract total USDT balance
    function getUSDTBalance(address user) public view returns (uint256) {
        return USDT.balanceOf(user);
    }
    function getERC3643TokenAddress()public view returns(address){
      return address(erc3643Token);
    }
}